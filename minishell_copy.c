...
int main(int argc, char **argv, char **envp)
{
    ...
    int last_exit_status = EXIT_SUCCESS; // Track the last command's exit status
    ...
    home = ft_strjoin("__HOME=", get_env_var_value_from_list(env_vars_list, "HOME"));
        ...
        if (*(heredoc_position) == '<' && *(heredoc_position + 1) == '<')
        {
            *heredoc_position = --heredoc_number;
            ...
            else if (child_pid == 0)  // Child process for heredoc reading
            {
                ...
                int tmp_file_fd = open(filename, O_CREAT | O_RDWR | O_TRUNC, 0644);
                free(filename);
        ...
        if (is_valid)
        {
            ...
            while (exit_status_var_pos)
            {
                *exit_status_var_pos = '\0';
                ft_strjoin_multiple(&input, last_exit_status_str, exit_status_var_pos + 2);
            ...
            while (env_var_pos)
            {
                ...
                *env_var_pos = '\0';
                char *var_value = get_env_var_value_from_list(env_vars_list, var_name);
                if (!var_value)
                    var_value = ft_strdup("");
            ...
            if (has_pipes(input))
            {
                ...
                last_exit_status = run_multiple_commands(commands, &env_vars_list);
