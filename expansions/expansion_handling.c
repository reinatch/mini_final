...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/25 14:51:24 by rerodrig          #+#    #+#             */
...
void	expand_present_exit_status_var(char **input, int last_exit_status)
{
	...
	while (exit_status_var_pos)
	{
		*exit_status_var_pos = '\0';
		str_join_3(input, last_exit_status_str, exit_status_var_pos + 2);
		exit_status_var_pos = find_exit_status_var_pos(*input);
	}
...
void	expand_present_env_vars(char **input, t_env_var_node *env_var_list)
{
	char	*env_var_pos;
	char	*var_name;
	char	*var_value;
	...
	if (env_var_pos)
	{
		...
		*env_var_pos = '\0';
		...
		if (!var_value)
			var_value = ft_strdup("");
		str_join_3(input, var_value, (env_var_pos + 1 + name_size));
		free(var_name);
		expand_present_env_vars(input, env_var_list);
	}
}
...
{
	expand_present_exit_status_var(input, last_exit_status);
	expand_present_env_vars(input, env_var_list);
}
